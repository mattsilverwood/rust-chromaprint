...
extern "C" {
    ...
    pub fn chromaprint_encode_fingerprint(
        ...
        encoded_fp: *mut *mut ::libc::c_void,
...
impl Chromaprint {
    ...
    pub fn fingerprint(&mut self) -> Option<String> {
        ...
        if unsafe { chromaprint_get_fingerprint(self.ctx, &mut fingerprint) } == 1 {
            let ret =
                String::from_utf8(unsafe { CStr::from_ptr(fingerprint) }.to_bytes().to_vec()).ok();
    ...
    pub fn encode(
        ...
        let result = unsafe {
            chromaprint_encode_fingerprint(
                raw_fingerprint.as_ptr() as *const ::libc::c_void,
                raw_fingerprint.len() as ::libc::c_int,
                algorithm,
                mem::transmute::<_, *mut *mut ::libc::c_void>(&mut array),
                &mut size,
                base64 as ::libc::c_int,
            )
        };
        if result == 1 {
    ...
    pub fn decode(
        ...
        base64: bool,
    ) -> Option<(Vec<::libc::c_int>, ChromaprintAlgorithm)> {
        let mut array: *mut ::libc::c_int = ptr::null_mut();
        let mut size: ::libc::c_int = 0;
        let mut algorithm: ChromaprintAlgorithm = -1;
        let result = unsafe {
            chromaprint_decode_fingerprint(
                encoded_fingerprint.as_ptr() as *const ::libc::c_void,
